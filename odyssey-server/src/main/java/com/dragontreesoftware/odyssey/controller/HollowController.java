...
import com.dragontreesoftware.odyssey.core.HollowReaderKey;
...
public class HollowController {
    ...
    public ResponseEntity<?> getTypes(HttpServletRequest request) {
        List<HollowReaderKey> types = hollowReaderService.getTypes();
        if(types != null) {
    ...
    public ResponseEntity<?> getKeysRange(HttpServletRequest request,
        ...
        List<HollowReaderKey> keys = hollowReaderService.getTypedKeys(type, from, count);
        if(keys != null) {
            return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON)
                    .body(keys);
        } else {
    ...
    public ResponseEntity<?> getKeys(HttpServletRequest request, @PathVariable String type) {
        List<HollowReaderKey> keys = hollowReaderService.getTypedKeys(type);
        if(keys != null) {
            return ResponseEntity.ok().contentType(MediaType.APPLICATION_JSON)
                    .body(keys);
        } else {
